...
def add_includes_to_package(package_file, includes, fonts, new_file, target):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if target == 'macos':
            # Move NexoBrowser/NexoBrowser.app (or Camoufox.app) -> NexoBrowser.app
            for nightly_name in ('NexoBrowser', 'Camoufox', 'nexobrowser', 'camoufox'):
                nightly_dir = os.path.join(temp_dir, nightly_name)
                for app_name in ('NexoBrowser.app', 'Camoufox.app', 'nexobrowser.app'):
                    app_path = os.path.join(nightly_dir, app_name)
                    if os.path.exists(app_path):
                        shutil.move(app_path, os.path.join(temp_dir, 'NexoBrowser.app'))
                        shutil.rmtree(nightly_dir)
                        break
        else:
            # Move contents out of the packaged app folder if it exists
            for folder_name in ('nexobrowser', 'camoufox', 'NexoBrowser'):
                old_dir = os.path.join(temp_dir, folder_name)
                if os.path.exists(old_dir) and os.path.isdir(old_dir):
                    staged = os.path.join(temp_dir, folder_name + '-folder')
                    os.rename(old_dir, staged)
                    for item in os.listdir(staged):
                        shutil.move(os.path.join(staged, item), temp_dir)
                    os.rmdir(staged)

        # Create target_dir
        if target == 'macos':
            target_dir = os.path.join(temp_dir, 'NexoBrowser.app', 'Contents', 'Resources')
        else:
            target_dir = temp_dir

        # User-visible Windows executable name
        if target == 'windows':
            for exe_name in ('nexobrowser.exe', 'camoufox.exe'):
                exe_path = os.path.join(target_dir, exe_name)
                branded = os.path.join(target_dir, 'NexoBrowser.exe')
                if os.path.exists(exe_path) and exe_path != branded:
                    os.rename(exe_path, branded)
                    break
...
def main():
    ...
    with temp_cd(src_dir):
        ...
        search_path = os.path.abspath(
            f'obj-{moz_target}/dist/*-{args.version}-{args.release}.*.{file_ext}'
        )
    ...
    package_pattern = f'*-{args.version}-{args.release}.en-US.*.{file_ext}'
    ...
    new_name = f'NexoBrowser-{args.version}-{args.release}-{args.os[:3]}.{args.arch}.zip'
